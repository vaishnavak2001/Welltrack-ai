...
if st.session_state.current_page == 'home':
    ...
    st.markdown(
        ...
        'AI-Powered Corporate Health Management Platform</p>',
        unsafe_allow_html=True,
    )
    ...
    if not st.session_state.logged_in:
        ...
        with st.form("health_assessment_form"):
            ...
            if submitted:
                ...
                # Numeric scores for persistence
                ...
                db = SessionLocal()
                try:
                    # Save the health record first
                    health_record = DatabaseOperations.save_health_record(
                        db, st.session_state.user_id, health_data
                    )
                    ...
                    risk_data = {
                        ...
                        'risk_category': (
                            'high' if diabetes_risk == "High" or heart_risk == "High"
                            else 'medium' if diabetes_risk == "Medium" or heart_risk == "Medium"
                            else 'low'
                        ),
                        'recommendations': (
                            'Immediate consultation recommended' if diabetes_risk == "High" or heart_risk == "High"
                            else 'Monitor closely' if diabetes_risk == "Medium" or heart_risk == "Medium"
                            else 'Continue healthy lifestyle'
                        ),
                    }

                    # Save risk assessment linked to the health record
                    DatabaseOperations.save_risk_assessment(
                        db, st.session_state.user_id, health_record.id, risk_data
                    )
                    ...
                    try:
                        ...
                        risk_notification_data = {
                            'diabetes': {'probability': diabetes_risk_score, 'category': diabetes_risk},
                            'heart_disease': {'probability': heart_risk_score, 'category': heart_risk},
                        }
                        notification_sent = send_notification_after_assessment(
                            st.session_state.user_id, risk_notification_data
                        )
...
# ------------------------------------------------------------------
    ...
    if not st.session_state.logged_in:
        ...
        try:
            ...
            with col2:
                format_type = st.selectbox("Export Format", ["PDF", "CSV"])

            # Check if user has any health records before offering download
            latest_record = DatabaseOperations.get_latest_health_record(db, st.session_state.user_id)

            if latest_record is None:
                st.info("📝 No health records found. Complete a health assessment first to generate reports.")
            else:
                if format_type == "CSV":
                    csv_data = report_gen.export_health_data_csv(st.session_state.user_id)
                    if csv_data:
                        st.download_button(
                            label="📥 Download CSV Report",
                            data=csv_data,
                            file_name=f"health_report_{date.today()}.csv",
                            mime="text/csv",
                    ...
                    pdf_buffer = report_gen.generate_health_report_pdf(st.session_state.user_id)
                    if pdf_buffer:
                        st.download_button(
                        ...
                        st.warning("No data available for PDF export.")

            st.markdown("---")

            # Show actual assessment history instead of fake data
            ...
            if health_history:
                history_rows = []
                for i, record in enumerate(health_history):
                    risk_level = "N/A"
                    if i < len(risk_assessments):
                        risk_level = risk_assessments[i].risk_category.capitalize()
                    history_rows.append({
                        'Date': record.recorded_at.strftime('%Y-%m-%d %H:%M'),
                        'BMI': f"{record.bmi:.1f}" if record.bmi else "N/A",
                        'BP': f"{record.systolic_bp}/{record.diastolic_bp}" if record.systolic_bp else "N/A",
                        'Glucose': f"{record.blood_glucose_fasting:.0f}" if record.blood_glucose_fasting else "N/A",
...
<div style='text-align: center; color: #666;'>
    <p>WellTrackAI © 2025 | Powered by AI | HIPAA Compliant</p>
</div>
""", unsafe_allow_html=True)
